...
function Exam() {
    ...
    const render = () => {
        ...
        fetch(`http://localhost:8080/exam/get-exams/${JSON.parse(localStorage.getItem('id'))}`, options)
            .then((res) => res.json())
            .then((res) => {
                setListExam(res);
            });
    ...
    const handleSearch = (e) => {
        if (e.keyCode === 13) {
            ...
            fetch(
                `http://localhost:8080/exam/get-exams/${JSON.parse(localStorage.getItem('id'))}?name=${e.target.value}`,
                options,
            )
                .then((res) => res.json())
                .then((res) => {
    ...
    const handleDelete = (id) => {
        ...
        fetch(`http://localhost:8080/exam/delete/${id}`, options).then(() => {
            render();
            notifyDelete();
        });
    };
