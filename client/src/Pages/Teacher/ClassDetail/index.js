...
function ClassDetail() {
    ...
    const render = () => {
        ...
        fetch(`http://localhost:8080/classroom/get-class/${localStorage.getItem('class_id')}`, options)
            .then((res) => res.json())
            .then((res) => {
                setClassroom((prev) => ({ ...prev, ...res }));
            });
        fetch(`http://localhost:8080/user/get-students/${localStorage.getItem('class_id')}`, options)
            .then((res) => res.json())
            .then((res) => {
    ...
    const search = (value) => {
        ...
        fetch(`http://localhost:8080/user/get-students/${localStorage.getItem('class_id')}?name=${value}`, options)
            .then((res) => res.json())
            .then((res) => {
                setListStudent(() => [...res]);
            });
    ...
    const handleRemove = (id) => {
        ...
        fetch(
            `http://localhost:8080/classroom/remove-student?` +
