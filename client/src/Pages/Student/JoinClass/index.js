...
function JoinClass() {
    ...
    const render = () => {
        ...
        fetch(
            `http://localhost:8080/classroom/get-joined-classes-by-student/${JSON.parse(localStorage.getItem('id'))}`,
            options,
        )
            .then((res) => res.json())
            .then((res) => {
    ...
    const handleQuitClass = (class_id) => {
        ...
        if (confirm) {
            ...
            fetch(
                `http://localhost:8080/classroom/remove-student?class_id=${class_id}&student_id=${JSON.parse(
